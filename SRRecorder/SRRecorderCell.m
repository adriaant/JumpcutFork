...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if ([aDecoder allowsKeyedCoding])
    {
        ...
        isASCIIOnly = [[aDecoder decodeObjectForKey:@"isASCIIOnly"] boolValue];
    }
    else
    {
...
//                              SRCharacterForKeyCodeAndCocoaFlags(keyCombo.code,keyCombo.flags));
                ...
                {
                    // invalid character or key combination
                    NSBeep();
                }
            }
        }
...
- (NSString *)keyChars
{
    if (!hasKeyChars) return (isASCIIOnly ? SRASCIIStringForKeyCode(keyCombo.code) : SRStringForKeyCode(keyCombo.code));
    return keyChars;
}
...
- (BOOL)_isEmpty
{
    return (![self _validModifierFlags:keyCombo.flags] || (isASCIIOnly ? !SRASCIIStringForKeyCode(keyCombo.code) : !SRStringForKeyCode(keyCombo.code)));
}
